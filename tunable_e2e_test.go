...
// .wasm and returns its path. The temp dir is cleaned up by the test
// framework, so no stray .wasm lands in the repo.
func buildWavetuneWasm(t *testing.T) string {
	t.Helper()
	return wasmfixtures.Build(t, "internal/wasmfixtures/wavetune")
}
