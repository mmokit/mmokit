...
func RegisterKind[T any](
	...
	for i := range bundleType.NumField() {
		...
		if universe.IsFrameworkCore(ct) {
			panic(fmt.Sprintf("mmokit.RegisterKind: bundle field %s.%s is a framework-core component (%s); the framework owns its wire format end to end — remove it from the bundle", bundleType.Name(), f.Name, ct.Name()))
		}
