...
**Status: implemented 2026-05-05, Plan G.** `mmokit.HandleClient[T]` is the registration verb for client-input handlers. Wire channel `0x02` carries typed Sends from connections; framework validates connection→player ownership and dispatches to the registered handler with `(player Entity, *msg)` — same signature as `HandleAll` / `HandleAllInternal`.
...
3. **Migrate remaining verbs.** Mining: **[done — 2026-05-04, Plan C]**. `gw.MineExtract(caster, asteroid, beam, requested)` replaces `sendCrossNodeMining`; `MiningAction`/`MiningResult` codec + `ActionMining` dispatch deleted; both `system_ability.go` (extract pulse) and `system_mining.go` (continuous beam) callsites migrated. StatusEffect: **[done — 2026-05-04, Plan D]**. `gw.ApplyStatus(caster, target, type, dur, val, slot, abilityType)` replaces `sendCrossNodeStatusEffect`; `StatusEffectAction` codec + `ActionStatusEffect` dispatch deleted; `system_ability.go` `AbilityTypeIonBurn` callsite migrated. Death/Loot/Currency: **[done — 2026-05-05, Plan E]**. `Killed{Killer}` typed message + death observer (`OnTickEachAll[Health]`) replace `processDeaths` hook + the imperative `if Current<=0 { MarkXxxDeath }` branch in `ApplyDamage`. `KillCredit{Currency, Amount}` (with `serverOnly()` marker) typed message replaces `gw.SideEffects.Emit(SideEffectCurrency, ...)` for cross-cell currency rewards — the Plan-D-flagged regression is closed by code. The §5 composition example exists in `internal/game/verb_death.go`. **Remaining:** target lock, dock requests — one plan per verb (F onward).
4. **[done — 2026-05-05, Plan E]** **Migrate ECS access.** All `gw.eng.ECS.X`, `gw.C.X.Get`, `gw.NetIDToEntity` sites swept across `internal/game/` and `cmd/server/`. Function signatures propagated from `ecs.Entity` to `mmokit.Entity` along the way. `gw.C` (the typed-mapper cache), `NewComponents`, and `gw.NetIDToEntity` field + the SpatialSystem hook that populated it all deleted.
...
6. **[done — 2026-05-05, Plan G]** **Migrate input handling.** `OnInput[T]` / `OnInputWith[T, Deps]` replaced by `mmokit.HandleClient[T]` registration verb. `ServerOnly()` marker interface replaced by paired `mmokit.HandleAllInternal[T]` verb — three registration verbs (HandleAll, HandleAllInternal, HandleClient) determine policy at the wiring site, not via marker methods on data types. `PlayerInputMsg` split into four discrete typed messages (`SetMoveTarget`, `SetLockTarget`, `CastAbility`, `JettisonItem`) — discrete events become discrete typed Sends. All gamepb input proto types deleted (`PlayerInputMsg`, `DockRequestMsg`, `UndockRequestMsg`, `RespawnRequestMsg`, `InventoryTransferMsg`, `BankRequestMsg`, `EquipRequestMsg`, `LootItemMsg`, `LootAllMsg`). Wire channel `0x02` carries client→server typed Sends; sdkgen emits TS classes per `HandleClient` registration with `encode()` + `client.send(msg)`. `pkg/mmokit/input.go`, `pkg/engine/input_dispatcher*`, `Process.AddInputBinding`/`InputBindings`/`dispatchInput` all deleted. Plus full cleanup sweep: dead `ActionResult` infrastructure deleted (`Bridge.SendActionResult`, `MsgActionResult`, `meshpb.ActionResult` proto + `MeshFrame.action_result` field 11), `internal/game/README.md` rewritten to describe post-Plan-E death pipeline, foundation deferral (c) `Set`-on-dead-entity log added, mining-beam press-pulse VFX restored via typed `BeamToggle`, codebase-wide `reflect.Ptr` → `reflect.Pointer` swept.

**Spec §10 migration plan: complete.** Plans A+B (foundation), C (Damage + Mining), D (StatusEffect + legacy CrossCellAction surface deletion), E (Death/Currency composition + ECS-access full sweep), F (AoI auto-broadcast + sdkgen TS classes), G (input migration + full cleanup sweep). All six steps landed; the entity-message-passing design is fully implemented in code.
