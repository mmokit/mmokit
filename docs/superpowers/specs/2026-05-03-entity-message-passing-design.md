...
3. **Migrate remaining verbs.** Mining: **[done — 2026-05-04, Plan C]**. `gw.MineExtract(caster, asteroid, beam, requested)` replaces `sendCrossNodeMining`; `MiningAction`/`MiningResult` codec + `ActionMining` dispatch deleted; both `system_ability.go` (extract pulse) and `system_mining.go` (continuous beam) callsites migrated. **Remaining:** status effects (`ActionStatusEffect`), target lock, dock requests, currency transfers — one plan per verb (E onward).
4. **Migrate ECS access.** Replace `gw.eng.ECS.Alive`, `gw.C.X.Get`, `gw.NetIDToEntity` mechanically across systems.
5. **Delete the old API surfaces.** `Bridge.SendAction` direct caller path, `CrossCellAction` opaque payload type, `SideEffectRegistry`, `action_codec.go`, `HandleCrossCellAction` switch, `HandleActionResult` switch. Once StatusEffect (the last `ActionType` user) migrates, the legacy switch reduces to just the engine-level `ActionTypedMessage` handler — at which point most of `action_codec.go` can disappear.
