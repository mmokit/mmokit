...
```bash
go list -deps github.com/zenion/mmoserver/pkg/mmokit 2>&1 | grep universe
go list -deps github.com/zenion/mmoserver/pkg/universe 2>&1 | grep mmokit
```
...
Add `enginepb "github.com/zenion/mmoserver/gen/go/enginepb"` to imports if not already present.
...
Add the import: `"github.com/zenion/mmoserver/pkg/ops"` (verify no cycle — `pkg/universe` likely doesn't import `pkg/ops` today; if it does cycle, declare the field as `any` like `Config.Protocol`).
