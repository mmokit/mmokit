...
2. **Per-cell dispatch** — currently the gateway's `forwardChannel` ships drained 0x00 bytes via MeshData to the host's `VirtualConnManager` (already typed-input-aware after the recent Plan G follow-up commit `a9c847f`). The host's existing `Stage.DispatchClientInput()` (called per tick) drains the per-session client-input queue; after Phase 5, that same queue receives 0x00 typed frames and the dispatcher just needs to not require the channel byte. Modify `Stage.DispatchClientInput` (or the helper it calls) to invoke `DispatchInboundEventFrame(stage, playerNetID, payload)` for each drained payload.
