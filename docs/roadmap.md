...
Escape hatches exist (`IncludeAll`, `Without` in `pkg/query/query.go`), but `ForEach1/2/3` in [`queries.go:41-71`](../queries.go) still build raw unfiltered filters, and WASM systems both read and overwrite neighbour-owned Replica and Ghost components (`wasm_system.go:186`, `:207`).
