...
// MarshalReplicaSnapshot encodes a ReplicaSnapshot to protobuf bytes.
func MarshalReplicaSnapshot(s *ReplicaSnapshot) ([]byte, error) {
	pb := &gamepb.ReplicaSnapshotPB{
		NetworkId:      s.NetworkID,
		EntityType:     uint32(s.EntityType),
		PosX:           s.Position.X,
		PosY:           s.Position.Y,
		SectorSx:       s.Sector.SX,
		SectorSy:       s.Sector.SY,
		VelX:           s.Velocity.X,
		VelY:           s.Velocity.Y,
		Rotation:       s.Rotation.Angle,
		ColliderRadius: s.Collider.Radius,
		ColliderWidth:  s.Collider.Width,
		ColliderHeight: s.Collider.Height,
		ColliderLayer:  uint32(s.Collider.Layer),
		ColliderShape:  uint32(s.Collider.Shape),
	}

	if s.Health != nil {
		pb.Health = &gamepb.HealthPB{Current: s.Health.Current, Max: s.Health.Max}
	}
	if s.Shield != nil {
		pb.Shield = &gamepb.ShieldPB{
			Current:       s.Shield.Current,
			Max:           s.Shield.Max,
			RegenRate:     s.Shield.RegenRate,
			RegenDelay:    s.Shield.RegenDelay,
			DamageCooldown: s.Shield.DamageCooldown,
		}
	}
	if s.Minable != nil {
		pb.Minable = &gamepb.MinablePB{
			ResourceType: uint32(s.Minable.ResourceType),
			Remaining:    s.Minable.Remaining,
		}
	}
...
// UnmarshalReplicaSnapshot decodes a ReplicaSnapshot from protobuf bytes.
func UnmarshalReplicaSnapshot(data []byte) (*ReplicaSnapshot, error) {
	...
	if pb.Health != nil {
		s.Health = &comp.Health{Current: pb.Health.Current, Max: pb.Health.Max}
	}
	if pb.Shield != nil {
		s.Shield = &comp.Shield{
			Current:        pb.Shield.Current,
			Max:            pb.Shield.Max,
			RegenRate:      pb.Shield.RegenRate,
			RegenDelay:     pb.Shield.RegenDelay,
