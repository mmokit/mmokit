...
// WriteStatusEffects serializes active status effects as a sequence of
// 2-byte records: [u8 type][qnorm duration/StatusEffectDurationScale].
// Inactive slots (i >= Count) are not emitted.
func WriteStatusEffects(se *gamecomp.StatusEffects, w *quantize.SnapshotWriter) {
...
// HashStatusEffects hashes active status effects using the same bytes
// WriteStatusEffects emits. Hashing raw Duration would cause false positives
// every tick as the float drifts smoothly even though the qnorm byte is
// unchanged, so we pre-quantize here to match the wire output exactly.
func HashStatusEffects(se *gamecomp.StatusEffects, h *system.Hasher) {
	for i := uint8(0); i < se.Count; i++ {
		eff := se.Effects[i]
...
func NewStatusEffectsBinding(m *ecs.Map1[gamecomp.StatusEffects]) system.ComponentBinding {
	return system.VarTailComponent(m, system.VarTailAccessor[gamecomp.StatusEffects]{
		...
		WriteItems: WriteStatusEffects,
		HashItems:  HashStatusEffects,
	})
}

// CountInventoryItems returns the number of inventory slots with positive qty.
func CountInventoryItems(inv *gamecomp.Inventory) int {
	count := 0
...
// WriteInventoryItems serializes inventory items as a sequence of 8-byte
// records sorted by item ID: [u32 itemID][u32 quantity]. Zero/negative
// quantities are skipped. Sorting is required so two inventories with
// identical contents produce identical wire bytes regardless of map order.
func WriteInventoryItems(inv *gamecomp.Inventory, w *quantize.SnapshotWriter) {
...
// HashInventoryItems hashes inventory items using the same sorted byte
// sequence WriteInventoryItems emits.
func HashInventoryItems(inv *gamecomp.Inventory, h *system.Hasher) {
	keys := sortedInventoryKeys(inv)
	for _, id := range keys {
		qty := inv.Items[id]
		if qty <= 0 {
...
func NewInventoryBinding(m *ecs.Map1[gamecomp.Inventory]) system.ComponentBinding {
	return system.VarTailComponent(m, system.VarTailAccessor[gamecomp.Inventory]{
		...
		Count:      CountInventoryItems,
		WriteItems: WriteInventoryItems,
		HashItems:  HashInventoryItems,
	})
}
