...
func (a *gameWorldAdapter) DispatchChat(username, text string) {
	a.gw.Log.Log(game.CatPlayerChat, "inbox: relayed chat <%s> %s", username, text)
...
func (a *gameWorldAdapter) HandleCrossNodeAction(action *mmokit.CrossNodeAction) *mmokit.ActionResult {
	...
	case game.ActionDamage:
		...
		if err != nil {
			gw.Log.Log(game.CatCombatAbility, "cross-node damage: bad payload from node=%s: %v", action.SourceNodeID, err)
			return nil
		}

		target, ok := gw.NetIDToEntity[action.TargetNetID]
		if !ok || !gw.ECS.Alive(target) {
			gw.Log.Log(game.CatCombatAbility, "cross-node damage: target netID=%d not found (from node=%s)",
		...
		gw.Log.Log(game.CatCombatAbility, "cross-node damage: src=%d -> target=%d dmg=%.1f dealt=%.1f (from node=%s)",
	...
	case game.ActionStatusEffect:
		...
		if err != nil {
			gw.Log.Log(game.CatCombatAbility, "cross-node status effect: bad payload from node=%s: %v", action.SourceNodeID, err)
			return nil
		}

		target, ok := gw.NetIDToEntity[action.TargetNetID]
		if !ok || !gw.ECS.Alive(target) {
			gw.Log.Log(game.CatCombatAbility, "cross-node status effect: target netID=%d not found (from node=%s)",
				action.TargetNetID, action.SourceNodeID)
			return nil
		}
		...
		if gw.C.StatusEffects.HasAll(target) {
			...
			gw.Log.Log(game.CatCombatAbility, "cross-node status effect: src=%d -> target=%d type=%d dur=%.1f val=%.1f (from node=%s)",
	...
	case game.ActionMining:
		mining, err := game.UnmarshalMiningAction(action.Payload)
		if err != nil {
			gw.Log.Log(game.CatEconomyMining, "cross-node mining: bad payload from node=%s: %v", action.SourceNodeID, err)
			return nil
		}

		target, ok := gw.NetIDToEntity[action.TargetNetID]
		if !ok || !gw.ECS.Alive(target) {
			gw.Log.Log(game.CatEconomyMining, "cross-node mining: target netID=%d not found (from node=%s)",
				action.TargetNetID, action.SourceNodeID)
			return nil
		}

		if !gw.C.Minable.HasAll(target) {
			gw.Log.Log(game.CatEconomyMining, "cross-node mining: target netID=%d not minable (from node=%s)",
				action.TargetNetID, action.SourceNodeID)
			return nil
		}
		...
		if depleted {
			...
			gw.Log.Log(game.CatEconomyMining, "cross-node mining: asteroid netID=%d depleted (from node=%s)",
				action.TargetNetID, action.SourceNodeID)
		} else {
			gw.Log.Log(game.CatEconomyMining, "cross-node mining: src=%d -> target=%d extracted=%.1f remaining=%.1f (from node=%s)",
		...
		result = &mmokit.ActionResult{
			...
			SourceNetID: action.SourceNetID,
			Success:     true,
			Payload:     game.MarshalMiningResult(&game.MiningResult{Extracted: extracted, Depleted: depleted}),
		}

	default:
		gw.Log.Log(game.CatCombatAbility, "cross-node action: unknown type=%d from node=%s", action.Type, action.SourceNodeID)
		return nil
	}
...
func (a *gameWorldAdapter) HandleActionResult(result *mmokit.ActionResult) {
	...
	if len(result.SideEffects) > 0 {
		...
		if err != nil {
			gw.Log.Log(game.CatCombatAbility, "cross-node side effects: bad data: %v", err)
		} else {
	...
	case game.ActionDamage:
		...
		if err != nil {
			gw.Log.Log(game.CatCombatAbility, "cross-node damage result: bad payload: %v", err)
			return
		}
		...
		gw.Log.Log(game.CatCombatAbility, "cross-node damage result: src=%d -> target=%d dealt=%.1f dead=%v",
			result.SourceNetID, result.TargetNetID, dmgResult.DamageDealt, dmgResult.TargetDead)

	case game.ActionStatusEffect:
		gw.Log.Log(game.CatCombatAbility, "cross-node status effect result: src=%d -> target=%d success=%v",
			result.SourceNetID, result.TargetNetID, result.Success)

	case game.ActionMining:
		miningResult, err := game.UnmarshalMiningResult(result.Payload)
		if err != nil {
			gw.Log.Log(game.CatEconomyMining, "cross-node mining result: bad payload: %v", err)
			return
		}
		...
		gw.Log.Log(game.CatEconomyMining, "cross-node mining result: target=%d extracted=%.1f depleted=%v",
