...
func RegisterConsoleCommands(reg *cmdsys.Registry, getRepo RepoProvider, disconnect DisconnectFn) error {
	...
	if err := must(reg.Register(cmdsys.Command{
		...
		Usage:       "auth user lock <username> <duration>",
	...
	if err := must(reg.Register(cmdsys.Command{
		...
		Usage:       "auth user unlock <username>",
	...
	if err := must(reg.Register(cmdsys.Command{
		...
		Usage:       "auth session list <username>",
	...
	if err := must(reg.Register(cmdsys.Command{
		...
		Usage:       "auth audit recent <username> [limit]",
