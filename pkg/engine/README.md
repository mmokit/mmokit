...
| `FlushRemovals(getNetID)` | Deletes all queued entities. The callback lets the game provide NetworkID lookup without the engine importing game types |
...
- `PendingAdminCmds chan AdminCmd` — admin command queue from console
