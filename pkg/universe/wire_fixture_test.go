...
// bindClientInputTypeID forces id → typ in w's client-input index, restoring
// the previous binding when the test ends.
//
...
// tests want.
func bindClientInputTypeID(tb testing.TB, w *WireRegistry, id uint32, typ reflect.Type) {
	tb.Helper()
	w.mu.Lock()
