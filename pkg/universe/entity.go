...
func EntityByNetID(stage *Stage, netID uint32) Entity {
...
// has no NetworkID.
func EntityFromECS(stage *Stage, h ecs.Entity) Entity {
...
func (e Entity) Local() bool {
	...
	return ok && presence == PresenceLive
}
...
func (e Entity) Stage() *Stage { return e.stage }
...
// Wire format: Entity encodes as 4-byte little-endian NetID. Decode
...
//
// Registered at init() so any package that imports universe transitively
// gets Entity round-tripping for free.
func init() {
	RegisterReflectCodec(reflect.TypeFor[Entity](), &ReflectCodec{
