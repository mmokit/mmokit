...
**Validation:** `pkg/universe/s6_gateway_test.go` (`TestS6HandoffAcrossNodes`) is the S6 capstone (login + cross-host handoff + disconnect). The S7 test family — `s7_split_test.go`, `s7_merge_test.go`, `s7_migrate_test.go`, `s7_graceful_shutdown_test.go`, `s7_concurrent_test.go` — covers split/merge/migrate across hosts, graceful drain, and concurrent handoff during split under the unified transfer protocol. The `examples/4node-basic` binary accepts all role combinations via `--mode=`; from inside `examples/4node-basic/` run `just distributed` to spin up 4 processes in a tmux session (coordinator + 2 hosts + gateway), or hand-roll with `--mode=coordinator --control-listen=:9100` + `--mode=host --coordinator-addr=localhost:9100 --host-id=NAME` + `--mode=gateway` (admin auto-binds on the coordinator at the default `:9101`). The space game has the equivalent `just distributed-space` at the repo root.
...
- Endpoints are registered on the gateway HTTP mux (RoleGateway processes) AND on the admin HTTP listener bound by `Config.AdminListen` / `--admin-listen` (default `:9101`, binds only on RoleCoordinator processes — pass `--admin-listen=""` to disable). The admin listener is the only HTTP surface on pure-coordinator processes (where commits actually run) since they don't bind the client port. See `examples/4node-basic just distributed` for the canonical distributed setup.
...
The SPA lives in `web-admin/` (Svelte 5 + Vite + Bun + Tailwind v4 + `@lucide/svelte`). `bun run build` outputs directly into `pkg/admin/static/dist/` so the binary's `//go:embed` picks it up. Local dev: `just admin-dev` runs Vite on `:5173` proxying API calls to a backend at `:9101`. CI/release: `just admin-build` regenerates the bundle; wired into the top-level `just build`. The Cluster page is canvas-rendered (`CellMap.svelte`) with quadtree-aware nesting; cell IDs use the canonical mesh form `cell_X_Y` (depth 0) / `cell_dN_X_Y` (depth N>0) — parse + format helpers live in [web-admin/src/lib/cellid.ts](web-admin/src/lib/cellid.ts), mirroring `pkg/universe/cell_id.go`. Click a cell → `CellDrawer` with split/merge/migrate actions. Live updates flow through one multiplexed SSE connection at `/admin/api/stream`. Stores using Svelte 5 runes live in `web-admin/src/lib/stores.svelte.ts` (the `.svelte.ts` extension is required for `$state` outside `.svelte` files).

**No browser-native dialogs.** Never use `window.confirm()`, `window.alert()`, or `window.prompt()` in the admin SPA — they look out of place, can't be styled, and break inside iframes/extensions. Use `ConfirmDialog.svelte` for yes/no confirmations (especially destructive ops like delete/kick), `ArgsModal.svelte` for typed-input prompts, or build a similar custom modal that matches the dashboard aesthetic. The `WorldInspector` delete flow is the canonical reference: a `$state` `open` flag, `onConfirm`/`onCancel` callbacks, danger styling on destructive actions.
...
- Struct tag encodings: `net:"qvel"` (explicit), `net:"auto"` (inferred from Go type), `net:"initial"` (variable-length field carried in the `initialData` blob — sent on visibility enter **and** re-sent whenever it changes; the replication system tracks a per-viewer initial-only hash and re-emits a full+initial frame on change, so editor/admin edits to fields like `Name` propagate live)
