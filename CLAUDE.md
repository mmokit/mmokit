...
- Never put a `net:` tag on a core component field (`Position`, `Velocity`, `Rotation`, `Collider`, `CellCoord`). `RegisterKind` rejects them as bundle fields via `IsFrameworkCore`, and the framework owns their wire format end to end — which is what lets those types widen (Z, quaternion, Depth) without moving a client byte. The two frames carry different sets, so there are two membership maps: `skipOnTransfer` (all five — `TransferFrame` carries the whole 18-byte collider) and `skipOnBorder` (four — the border header carries the collider's *radius* alone, so `Collider` rides the border component tail).
