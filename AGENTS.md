...
- An example's `just run`, `just dev`, `just distributed`, `just web-serve` and (space only) `just botclient`, plus the root `just admin-dev`, `just csharp-smoke`, `just csharp-cli` and the prometheus recipes, are long-running/manual workflows. Run them only when useful and stop any tmux/server sessions you start.
...
| Entity/event/input/op schema | `just schema-check` first — it byte-diffs all three examples' `--dump-schema` against `testdata/schema/`, so it fails on any client-visible protocol change including ones you did not intend. If the change is intended, `just schema-golden`, then regenerate each affected SDK, inspect the diff, and run the corresponding frontend checks. Needs PostgreSQL (`just db-up`) |
...
| C# core/generator | `just csharp-test`; generator changes also run `just csharp-compile-test`, and changes to `UdpTransport.Connect`/`MmokitAuth`/the generated client also run `just csharp-smoke-build` and `just csharp-cli-build` (the only gates that compile *consumers* of a generated SDK — the smoke bot sat broken for three commits because neither other gate builds it). Note that a compile gate cannot see a wrong REQUEST: the generated `ConnectAsync` omitted the schema fingerprint and every gate stayed green while no C# client could open a UDP session. Only `just csharp-cli` or `just csharp-smoke` against a live server catches that class; regenerate goldens when wire bytes intentionally change |
