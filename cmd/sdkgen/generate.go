...
// schemaUsesQuat reports whether any field in the schema carries the qquat
// encoding.
//
// The generated imports are CONDITIONAL on it, and that is the whole reason
// this exists: importing unQuat unconditionally would add a line to every
// generated SDK including the 2D ones, and phase 2's rule is that a 2D game
// pays nothing for a 3D feature. The gate on a regeneration is that
// examples/space and examples/4node-basic diff to zero.
func schemaUsesQuat(schema ProtocolSchema) bool {
...
func (g *Generator) genEntities() string {
	...
	if schemaUsesQuat(g.schema) {
		b.WriteString("import type { Quat } from \"./_core/delta-decoder-core.js\";\n\n")
	}
...
func encodingToTSType(enc string) string {
	...
	case "bool":
		return "boolean"
	case "qquat":
		// The first non-scalar encoding. It stays ONE schema field with one
		// bound name and gains an object type; splitting it into four
		// synthesized scalars would force this generator to invent names the
		// schema does not carry.
		return "Quat"
	case "f32", "qvel", "qangle", "qnorm", "u8", "u16", "u32", "i16":
		return "number"
	default:
		// Loud, not "number". A silent fallback here is how a new encoding
		// ships as a plain number in every generated client while the server
		// emits something else entirely — the decode-line emitters below
		// already panic, and this mapper not doing so was the asymmetry.
		panic(fmt.Sprintf("sdkgen: unsupported field encoding %q — add it to encodingToTSType", enc))
	}
}
...
func (g *Generator) genDeltaDecoder() string {
	...
	if schemaUsesQuat(g.schema) {
		b.WriteString("  unQuat, QUAT_WIRE_SIZE,\n")
	}
...
func writeFieldDecoderIndented(b *strings.Builder, field BindingSchemaField, indent string) {
	...
	case "qquat":
		fmt.Fprintf(b, "%sconst %s = unQuat(snap, o); o += QUAT_WIRE_SIZE;\n", indent, field.Name)
	case "qnorm":
...
func writeFieldDecoder(b *strings.Builder, field BindingSchemaField) {
	...
	case "qquat":
		fmt.Fprintf(b, "  const %s = unQuat(snap, o); o += QUAT_WIRE_SIZE;\n", field.Name)
	case "qnorm":
...
func writeInitialFieldDecoder(b *strings.Builder, field BindingSchemaField, _ string) {
	...
	default:
		...
		// which desynchronizes every initial field after it — a corruption
		// that reads as plausible values rather than an error.
		panic(fmt.Sprintf("sdkgen: unsupported initial-field encoding %q for field %q", field.Encoding, n))
	}
}
