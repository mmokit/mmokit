...
func (g *Generator) genEntities() string {
	...
	// Client render mode: declared by the server's protocol schema and
	// emitted as a narrowed `as const` so game code that branches on the
	// constant gets compile-time dead-code elimination via TypeScript's
	// control-flow analysis.
	mode := g.schema.ClientRenderMode
	if mode == "" {
		mode = "interpolated"
	}
	b.WriteString("/**\n")
	b.WriteString(" * Client render mode declared by the server's protocol schema.\n")
	b.WriteString(" *\n")
	b.WriteString(" * - \"interpolated\": render-lag interpolation between server samples\n")
	b.WriteString(" *   PLUS client-side prediction for the local player. Smooth motion\n")
	b.WriteString(" *   at 60fps, clicks feel instant, but predicted state must reconcile\n")
	b.WriteString(" *   with server — can show rubber-band / hitch artifacts at direction\n")
	b.WriteString(" *   changes and cell boundaries.\n")
	b.WriteString(" *\n")
	b.WriteString(" * - \"snap\": render-lag interpolation between server samples ONLY.\n")
	b.WriteString(" *   No client-side prediction for the local player — clicks send to\n")
	b.WriteString(" *   the server and the player waits for server confirmation before\n")
	b.WriteString(" *   moving. Motion stays smooth at 60fps for all entities (including\n")
	b.WriteString(" *   the local player) because the sample ring still interpolates;\n")
	b.WriteString(" *   only prediction is disabled. League-of-Legends authority model.\n")
	b.WriteString(" *\n")
	b.WriteString(" * Game code should branch on this constant to enable/disable the\n")
	b.WriteString(" * prediction path. Interpolation runs in both modes.\n")
	b.WriteString(" */\n")
	fmt.Fprintf(&b, "export const CLIENT_RENDER_MODE = %q as const;\n", mode)
	b.WriteString("export type ClientRenderMode = typeof CLIENT_RENDER_MODE;\n\n")
	// Cast to `string` in the comparison so the `as const` narrowing on
	// CLIENT_RENDER_MODE doesn't trigger TS2367 ("no overlap") on whichever
	// branch isn't the schema's compiled-in value. The constant still
	// narrows at callers — `if (isSnapMode()) { ... }` collapses via DCE
	// when CLIENT_RENDER_MODE is known to be `"interpolated"` at compile
	// time, because the function body is trivially reachable but its
	// return value is statically knowable.
	b.WriteString("export function isSnapMode(): boolean {\n")
	b.WriteString("  return (CLIENT_RENDER_MODE as string) === \"snap\";\n")
	b.WriteString("}\n\n")
	b.WriteString("export function isInterpolatedMode(): boolean {\n")
	b.WriteString("  return (CLIENT_RENDER_MODE as string) === \"interpolated\";\n")
	b.WriteString("}\n\n")
