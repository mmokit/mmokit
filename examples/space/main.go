...
import (
	...
	gamecommands "github.com/zenion/mmokit/examples/space/internal/game/commands"
	...
	gamepg "github.com/zenion/mmokit/examples/space/internal/persist/postgres"
	"github.com/zenion/mmokit/examples/space/internal/world/jsonrepo"
...
func main() {
	...
	// built into examples/space/web/dist and served by a separate static host (see
	...
	// Relative to the process's working directory, which for `just run` is this
	// example's own directory. jsonrepo.LoadAll MkdirAll's this path, so a wrong
	// default does not error — it silently creates an empty manifest directory
	// wherever the binary happened to start. The example's justfile passes an
	// absolute --world-dir so running from anywhere else still finds the real one.
	...
	if needsGameConfig {
		// Precedence: --postgres-url (bound by coordCfg.BindFlags), then the
		// POSTGRES_URL env var, then the local docker-compose default.
		//
		// The flag arm is not decoration. BindFlags registers --postgres-url
		// regardless, so before this existed the flag was accepted and then
		// silently discarded — `just client-sdk examples/space` passes it, and
		// so does anyone who reads --help.
		postgresURL := coordCfg.PostgresURL
		if postgresURL == "" {
			postgresURL = os.Getenv("POSTGRES_URL")
		}
		if postgresURL == "" {
			postgresURL = "postgres://mmo:mmo@localhost:5432/mmo_space?sslmode=disable"
		}
		var err error
