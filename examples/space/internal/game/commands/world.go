...
func registerWorldExport(reg *cmdsys.Registry, coord *mmokit.Process, we *worldEditor) error {
	return reg.Register(cmdsys.Command{
		...
		Handler: func(ctx context.Context, env *cmdsys.Env, _ any) (any, error) {
			...
			return WorldExportResult{Path: repoDir(we.repo)}, nil
		},
	})
}
...
// repoDir reports where a world.Repository stores its manifests, for operator
// feedback from world.export. The interface does not require a location — an
// in-memory or database-backed implementation has none — so this is an
// optional-interface probe with an honest fallback rather than a hardcoded
// "world/", which stopped being true when this game moved under examples/.
func repoDir(r world.Repository) string {
	if d, ok := r.(interface{ Dir() string }); ok {
		return d.Dir()
	}
	return "(repository location not reported)"
}
